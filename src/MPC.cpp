...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for( int i = 0; i < N; i++ ) {
      fg[0] += weight_refer * CppAD::pow(vars[cte_start + i] - ref_cte, 2);
      fg[0] += weight_refer * CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
      fg[0] += CppAD::pow(vars[v_start + i] - ref_v, 2);
    }

    // Minimize the use of actuators.
    for (int i = 0; i< N - 1; i++) {
      fg[0] += weight_actuator * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += weight_actuator * CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the value gap between sequential actuations.
    // (how smooth the actuations are)
    for (int i = 0; i < N - 2; i++) {
      fg[0] += weight_angle_rate * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += weight_a_rate * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
